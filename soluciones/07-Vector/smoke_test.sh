#!/usr/bin/env bash
# smoke_test.sh — Prueba de humo automatizada para 07-Vector
set -euo pipefail
...
echo -e "${BLUE}==================================================${NC}"
echo -e "${BLUE}🚀 Iniciando Prueba de Humo: Stack Vector (VLGA)${NC}"
echo -e "${BLUE}==================================================${NC}"
...
TEST_MSG="SMOKETEST_VECTOR_$(date +%s)_$RANDOM"
...
for i in {1..15}; do
  ...
  if echo "$RESULT" | grep -q "$TEST_MSG"; then
    INDEXED=true
    echo -e "${GREEN}🎉 ¡ÉXITO! Log encontrado indexado en Loki via Vector!${NC}"
    break
  fi
