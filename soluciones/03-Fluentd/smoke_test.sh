#!/usr/bin/env bash
# smoke_test.sh — Prueba de humo automatizada para 03-Fluentd
set -euo pipefail
...
echo -e "${BLUE}==================================================${NC}"
echo -e "${BLUE}🚀 Iniciando Prueba de Humo: Stack Fluentd (EFK)${NC}"
echo -e "${BLUE}==================================================${NC}"
...
TEST_MSG="SMOKETEST_FLUENTD_$(date +%s)_$RANDOM"
...
# Paso 4: Esperar a que se indexe en Elasticsearch
echo -e "${YELLOW}⏳ Buscando el log en Elasticsearch (puerto 9200)...${NC}"
INDEXED=false
for i in {1..15}; do
  RESULT=$(curl -s "http://localhost:9200/_search?q=$TEST_MSG" || true)
  if echo "$RESULT" | grep -q "$TEST_MSG"; then
    INDEXED=true
    echo -e "${GREEN}🎉 ¡ÉXITO! Log encontrado indexado en Elasticsearch via Fluentd!${NC}"
    break
  fi
...
if [ "$INDEXED" = "false" ]; then
  echo -e "${RED}❌ ERROR: El log no fue indexado en Elasticsearch a tiempo.${NC}"
  exit 1
fi
