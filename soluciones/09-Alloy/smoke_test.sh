#!/usr/bin/env bash
# smoke_test.sh — Prueba de humo automatizada para 09-Alloy
set -euo pipefail
...
echo -e "${BLUE}==================================================${NC}"
echo -e "${BLUE}🚀 Iniciando Prueba de Humo: Stack Grafana Alloy (LGA)${NC}"
echo -e "${BLUE}==================================================${NC}"
...
TEST_MSG="SMOKETEST_ALLOY_$(date +%s)_$RANDOM"
...
# Paso 4: Esperar a que se indexe en Loki (puerto 3100)
echo -e "${YELLOW}⏳ Buscando el log en Loki (puerto 3100)...${NC}"
INDEXED=false
for i in {1..15}; do
  RESULT=$(curl -G -s --data-urlencode "query={job=~\".+\"} |= \"$TEST_MSG\"" http://localhost:3100/loki/api/v1/query_range || true)
  if echo "$RESULT" | grep -q "$TEST_MSG"; then
    INDEXED=true
    echo -e "${GREEN}🎉 ¡ÉXITO! Log encontrado indexado en Loki via Grafana Alloy!${NC}"
    break
  fi
