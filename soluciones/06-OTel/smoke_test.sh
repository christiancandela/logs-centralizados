#!/usr/bin/env bash
# smoke_test.sh — Prueba de humo automatizada para 06-OTel
set -euo pipefail
...
echo -e "${BLUE}==================================================${NC}"
echo -e "${BLUE}🚀 Iniciando Prueba de Humo: Stack OTel + LGTM${NC}"
echo -e "${BLUE}==================================================${NC}"
...
TEST_MSG="SMOKETEST_OTEL_$(date +%s)_$RANDOM"
...
# Paso 4: Esperar a que se indexe en el Loki interno (a través del proxy de Grafana en el puerto 3000)
echo -e "${YELLOW}⏳ Buscando el log en Loki (vía proxy de Grafana en puerto 3000)...${NC}"
INDEXED=false
for i in {1..15}; do
  RESULT=$(curl -u admin:admin -G -s --data-urlencode "query={service_name=\"logs-producer\"} |= \"$TEST_MSG\"" http://localhost:3000/api/datasources/proxy/uid/loki/loki/api/v1/query_range || true)
  if echo "$RESULT" | grep -q "$TEST_MSG"; then
    INDEXED=true
    echo -e "${GREEN}🎉 ¡ÉXITO! Log encontrado indexado en Loki via OTel Collector!${NC}"
    break
  fi
...
if [ "$INDEXED" = "false" ]; then
  echo -e "${RED}❌ ERROR: El log no fue indexado en Loki a tiempo.${NC}"
  exit 1
fi
