#!/usr/bin/env bash
# smoke_test.sh — Prueba de humo automatizada para 04-Promtail
set -euo pipefail
...
echo -e "${BLUE}==================================================${NC}"
echo -e "${BLUE}🚀 Iniciando Prueba de Humo: Stack Promtail (PLG)${NC}"
echo -e "${BLUE}==================================================${NC}"
...
TEST_MSG="SMOKETEST_PROMTAIL_$(date +%s)_$RANDOM"
...
# Paso 4: Esperar a que se indexe en Loki
...
for i in {1..15}; do
  ...
  if echo "$RESULT" | grep -q "$TEST_MSG"; then
    INDEXED=true
    echo -e "${GREEN}🎉 ¡ÉXITO! Log encontrado indexado en Loki!${NC}"
    break
  fi
