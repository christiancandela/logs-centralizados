#!/usr/bin/env bash
# smoke_test.sh — Prueba de humo automatizada para 01-ELK
set -euo pipefail
...
echo -e "${BLUE}==================================================${NC}"
echo -e "${BLUE}🚀 Iniciando Prueba de Humo: Stack ELK${NC}"
echo -e "${BLUE}==================================================${NC}"
...
TEST_MSG="SMOKETEST_ELK_$(date +%s)_$RANDOM"
...
for i in {1..15}; do
  ...
  if echo "$RESULT" | grep -q "$TEST_MSG"; then
    INDEXED=true
    echo -e "${GREEN}🎉 ¡ÉXITO! Log encontrado indexado en Elasticsearch!${NC}"
    break
  fi
